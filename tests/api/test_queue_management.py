...
def fake_redis():
    """Injects a fresh FakeRedis into the app for every test and cleans up after."""
...
def test_list_members_empty_queue(client: TestClient, db_session: Session, fake_redis):
...
def test_list_members_returns_all_in_order(client: TestClient, db_session: Session, fake_redis):
    tenant, queue = make_tenant_and_queue(db_session)
    for i, nome in enumerate(["Alice", "Bob", "Charlie"]):
        add_member(fake_redis, tenant.id, queue.id, {"nome": nome}, offset=i * 0.001)
...
def test_remove_member_success(client: TestClient, db_session: Session, fake_redis):
    tenant, queue = make_tenant_and_queue(db_session)
    user_data = {"nome": "Alice"}
    add_member(fake_redis, tenant.id, queue.id, user_data)
...
def test_call_next_pops_first_member(client: TestClient, db_session: Session, fake_redis):
    tenant, queue = make_tenant_and_queue(db_session)
    add_member(fake_redis, tenant.id, queue.id, {"nome": "First"}, offset=0)
    add_member(fake_redis, tenant.id, queue.id, {"nome": "Second"}, offset=0.001)
...
def test_clear_queue_removes_all_members(client: TestClient, db_session: Session, fake_redis):
    tenant, queue = make_tenant_and_queue(db_session)
...
def test_get_queue_size(client: TestClient, db_session: Session, fake_redis):
    tenant, queue = make_tenant_and_queue(db_session)
    add_member(fake_redis, tenant.id, queue.id, {"nome": "X"}, offset=0)
    add_member(fake_redis, tenant.id, queue.id, {"nome": "Y"}, offset=0.001)
...
# ──────────────────────────────────────────────
# Tests — Reorder Member
# ──────────────────────────────────────────────

def test_reorder_member_success(client: TestClient, db_session: Session, fake_redis):
    tenant, queue = make_tenant_and_queue(db_session)
    for i, nome in enumerate(["Alice", "Bob", "Charlie"]):
        add_member(fake_redis, tenant.id, queue.id, {"nome": nome}, offset=i * 0.001)

    resp = client.put(
        f"/api/v1/b2b/queue/{queue.id}/members/reorder",
        json={"user_data": {"nome": "Charlie"}, "target_position": 0},
        headers=auth_headers(tenant.id)
    )
    assert resp.status_code == 200
    assert resp.json()["status"] == "member_reordered"
    assert resp.json()["new_position"] == 0


def test_reorder_member_not_found_returns_400(client: TestClient, db_session: Session):
    tenant, queue = make_tenant_and_queue(db_session)

    resp = client.put(
        f"/api/v1/b2b/queue/{queue.id}/members/reorder",
        json={"user_data": {"nome": "Nobody"}, "target_position": 0},
        headers=auth_headers(tenant.id)
    )
    assert resp.status_code == 400
    assert "reorder" in resp.json()["detail"].lower()
